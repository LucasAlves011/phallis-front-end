...
// --- DADOS DO MOCK (Extraídos de lib/productData.ts) ---
...
async function migrate() {
   ...
   try {
      ...
      console.error('Erro de conexão ao logar:', e);
      process.exit(1);
   }
   ...
   for (const produto of produtosDoCatalogo) {
      ...
      // Se a imagem é local (começa com /images/), vamos tentar fazer upload
      if (produto.imageUrl && produto.imageUrl.startsWith('/images/')) {
         const localImagePath = path.join(__dirname, '..', 'public', produto.imageUrl);

         if (fs.existsSync(localImagePath)) {
            console.log(`   Upload da imagem: ${localImagePath}`);

            try {
               const fileBuffer = fs.readFileSync(localImagePath);
               const blob = new Blob([fileBuffer]);
               const formData = new FormData();
               formData.append('file', blob, path.basename(localImagePath));
               ...
               if (uploadRes.ok) {
                  ...
                  // O backend deve retornar algo como { fileDownloadUri: "..." }
                  if (uploadData.fileDownloadUri) {
                     finalImageUrl = uploadData.fileDownloadUri;
                     console.log(`   Imagem enviada com sucesso! Nova URL: ${finalImageUrl}`);
                  } else {
                     console.warn('   Upload feito, mas resposta sem fileDownloadUri:', uploadData);
                  }
               } else {
                  console.error('   Falha no upload:', await uploadRes.text());
               }

            } catch (err) {
               console.error('   Erro ao ler/enviar arquivo:', err);
            }
         } else {
            console.warn(`   Arquivo de imagem não encontrado localmente: ${localImagePath}`);
         }
      }

      // 3. CADASTRAR/ATUALIZAR O PRODUTO
      // Vamos tentar um POST. Se o ID já existir, pode dar erro ou criar duplicado dependendo do backend.
      // O ideal seria verificar se existe, mas vamos tentar POST direto.
      // Ajustamos os dados para enviar a URL final da imagem

      const payload = {
         ...produto,
         imageUrl: finalImageUrl,
         // Alguns ajustes podem ser necessários dependendo da DTO do backend
         // O backend espera 'options' como objeto? Sim, deve ser igual ao JSON.
      };

      // Removemos o ID original para deixar o banco gerar (ou mantemos se o backend aceitar define ID)
      // Se o backend gera ID auto-incremento (Long), enviar String "prod_001" vai falhar.
      // VAMOS TENTAR REMOVER O ID para criar novos.
      delete payload.id;

      console.log('   Enviando dados do produto...');
      try {
         const createRes = await fetch(`${API_URL}/api/produtos`, {
            method: 'POST',
            headers: {
               'Content-Type': 'application/json',
               'Authorization': `Bearer ${token}`
         ...
         if (createRes.ok) {
            ...
            console.error('   Erro ao criar produto:', await createRes.text());
         }
